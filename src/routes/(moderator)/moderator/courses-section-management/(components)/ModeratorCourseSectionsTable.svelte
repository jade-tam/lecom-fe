<script lang="ts">
	...
	import { type ModeratorPendingSection } from '$lib/types/Course';
	import { DataTable } from '@careswitch/svelte-data-table';

	const { sections }: { sections: ModeratorPendingSection[] } = $props();
	...
	const table = $derived(
		new DataTable({
			...
			columns: [
				...
				{ id: 'title', key: 'title', name: 'Tiêu đề chương' }
			]
		})
	);
</script>
...
<div class="my-2 flex items-end justify-between">
	<AnimatedDiv animateVars={{ translateX: -16 }} class="flex flex-col">
		<h1 class="text-header3 font-bold">Chương chờ duyệt</h1>
		<p class="text-base-content/60">
			Xem thông tin chương và thực hiện thao tác duyệt hoặc từ chối.
		</p>
	</AnimatedDiv>
								...
								<div class="flex justify-end gap-1">
									<div class="tooltip tooltip-top" data-tip="Chấp thuận">
										<FormConfirmPopoverButton
											action="?/approve"
											formData={{ id: row.id }}
											popoverId="approve-section-{row.id}"
											openButtonProps={{ class: 'btn btn-success' }}
											dropdownClass="dropdown-bottom dropdown-end"
											dropdownContent={{
												label: 'Chấp thuận chương này?',
												description: 'Chương sẽ được hiển thị trên khóa học của người bán.',
									...
									<div class="tooltip tooltip-top" data-tip="Từ chối">
										<FormConfirmPopoverButton
											action="?/reject"
											formData={{ id: row.id }}
											popoverId="reject-section-{row.id}"
											openButtonProps={{ class: 'btn btn-error' }}
											dropdownClass="dropdown-bottom dropdown-end"
											dropdownContent={{
												label: 'Từ chối chương này?',
												description: 'Chương sẽ không được hiển thị.',
				...
				{:else}
					<tr>
						<td colspan={table.columns.length + 1}>
							<EmptyPlaceholder
								text="Không có chương chờ duyệt."
								description="Tất cả chương đã được kiểm duyệt."
							/>
						</td>
					</tr>
				{/if}
