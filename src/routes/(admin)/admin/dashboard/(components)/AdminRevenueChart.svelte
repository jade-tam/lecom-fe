<script lang="ts">
	...
	const {
		chartData
	}: { chartData: { date: Date; revenue: number; platformFee: number; orderCount: number }[] } =
		$props();
</script>
		...
		{#snippet children({ data })}
			...
			<Tooltip.List>
				...
				<Tooltip.Item
					...
					value={formatVND(data.platformFee)}
				/>
				<Tooltip.Item label="Số đơn hàng" class="text-primary-content" value={data.orderCount} />
			</Tooltip.List>
		{/snippet}
	</Tooltip.Root>
</Chart>
