...
<div class="my-2 flex items-end justify-between">
	<AnimatedDiv animateVars={{ translateX: -16 }} class="flex flex-col">
		...
		<p class="text-base-content/60">Xem và quản lý các yêu cầu hoàn tiền từ khách hàng của bạn.</p>
	</AnimatedDiv>
						...
						<tr>
							...
							<td>
								<div class="flex gap-1">
									{#if row.status === 'PendingShop'}
										<div class="tooltip tooltip-left" data-tip="Xét duyệt hoàn tiền">
											<OpenModalButton
												openButtonProps={{ class: 'btn btn-warning btn-square' }}
												ModalContent={ReviewRefundModalContent}
												ModalContentProps={{ dataForm: reviewRefundFormData, refund: row }}
												><span class="icon-[mingcute--checkbox-line] text-xl"></span>
											</OpenModalButton>
										</div>
									{/if}
									<div class="tooltip tooltip-left" data-tip="Xem chi tiết đơn">
										<a
											href={resolve(`/(seller)/seller/orders/[id]`, { id: row.orderId })}
