...
<dialog bind:this={linkProductModalRef} class="modal">
    <div class="modal-box">
        <form action="?/linkProduct" method="POST" use:linkProductEnhance>
            ...
            {:then products}
                {#if products.length}
                    <div class="mt-1 flex max-h-64 overflow-auto w-full flex-col gap-1 rounded-field border p-1">
                            ...
                            <div class="flex gap-1">
                                ...
                                >
                                    <Image
                                        ...
                                        class="h-6 w-6"
                                    />
                                    <p class="line-clamp-1 max-md:max-w-52 max-w-84">{product.name}</p>
    ...
    <div class="mt-4 h-fit w-full rounded-box border bg-base-100 p-4">
        ...
        {:then sections}
            {#if sections.length}
                    ...
                    <div class="mt-2 rounded-box bg-base-200 p-4">
                        ...
                        <div class="mt-2 rounded-field bg-base-300">
                            {#if section.lessons.length}
                                ...
                                {/each}
                            {:else}
                                <p class="italic p-2 text-xs text-base-content/60">
                ...
                {/each}
            {:else}
                <p class="italic p-2 text-xs text-base-content/60">Chưa có chương, hãy thêm chương mới</p>
            {/if}
