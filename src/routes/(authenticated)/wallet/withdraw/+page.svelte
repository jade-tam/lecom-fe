<script lang="ts">
	...
	import CustomerWithdrawalsTable from './(components)/CustomerWithdrawalsTable.svelte';
...
<div class="my-2 flex flex-wrap justify-between gap-2">
	<AnimatedDiv animateVars={{ translateX: -20 }} class="flex flex-col">
		...
		<p class="text-base-content/60">
			Danh sách yêu cầu rút tiền từ ví cá nhân về tài khoản ngân hàng của bạn.
		</p>
	</AnimatedDiv>

	<AnimatedDiv animateVars={{ translateX: 16 }} class="flex gap-2">
		<a href={resolve('/wallet/withdraw/create')} class="btn btn-primary">
			<span class="icon-[mingcute--add-circle-line] text-xl"></span>Tạo yêu cầu mới</a
		>
	</AnimatedDiv>
</div>

<CustomerWithdrawalsTable withdrawalsPromise={data.myWithdrawalsPromise} />
